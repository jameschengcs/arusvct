...
print('rec_denoise_mdl:', rec_denoise_mdl)
if not os.path.exists(rec_denoise_mdl):
    arg_chk = False
    print('rec_denoise_mdl is not existed')
...
# Initializing models
sino_syn = HDNet(residual = True)  
sino_syn.to(device)
sino_syn.load_state_dict(torch.load(sino_syn_mdl))
sino_syn.eval() 

rec_denoise = HDNet(residual = True)  
rec_denoise.to(device)
rec_denoise.load_state_dict(torch.load(rec_denoise_mdl))
rec_denoise.eval()
...
for file_id in range(n_files):
    ...
    output_path = output_dir + '_' + filename + '.npy'
...
# Loading Phase 1 images
patch_layout = (1, 1)
rec_ds = CtDataset(output_dir, None,
                  patch_layout = patch_layout,
                  device = device,
                  mem = False)
print('rec_shape', rec_shape, flush=True)
batch_size = 1 
n_batches = n_sino_per_file // batch_size
print('Denoising images', flush=True)
for file_id in range(n_files):
    filename = rec_ds.filenames[file_id]
    rec_imgs = np.zeros(rec_imgs_shape, dtype = np.float32)
    for i in range(n_batches):
        img_id = i * batch_size
        img_ide = img_id + batch_size
        img_batch = rec_ds.feed(model = rec_denoise, file_id = file_id, image_id = img_id, batch_size = batch_size)
        rec_imgs[img_id:img_ide] = img_batch.astype(np.float32)
    
    rec_imgs = postproc(rec_imgs, mask = mask, clip_min = True, clip_max = False, mean_align = True, ref_mean = org_vranges[file_id][2]) 
    org_filename = sino_ds.filenames[file_id]
    output_path = output_dir + org_filename + '.npy'
    np.save(output_path, rec_imgs)    
    print('Resulting images saved:', output_path, flush=True)
print('Image denoising finished, time:', time.time() - time_s, flush=True)
