...
from torch.nn import Module, ModuleList, Sequential, Sigmoid, Hardtanh, Conv2d, Upsample, Linear, MaxPool2d, LeakyReLU, ReLU, Dropout, AdaptiveAvgPool2d, Flatten, ConvTranspose2d, BatchNorm2d
import numpy as np
...
class up_conv(nn.Module):
    def __init__(self,ch_in,ch_out):
        ...
        self.up = nn.Sequential(
            ...
            #ConvTranspose2d(ch_in,ch_out, kernel_size=3, stride=2, padding=1, output_padding = 1),
            ConvTranspose2d(ch_in,ch_out, kernel_size = 4, stride=2, padding=1),
            BatchNorm2d(ch_out),
            LeakyReLU(negative_slope = negative_slope, inplace=True),
...
class Decoder(Module):
    def __init__(self, chs=(1024, 512, 256, 128, 64)):
        ...
        self.upconvs    = ModuleList([up_conv(chs[i], chs[i+1]) for i in range(len(chs)-1)])
