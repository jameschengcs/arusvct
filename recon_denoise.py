...
from aru import ARUNet, ARUNet3, ARUNet2, DUNet, init_weights
...
# Model ID:
# 'hd': the HDNet
# 'du': the proposed U-Net for image denoising
# 'ru': the residual U-Net
# 'aru': the proposed U-Net for sinogram synthesis

# The model of sinogram synthesis
pre_model_id = 'aru' if n_args < 2 else sys.argv[1] 

# The model of reconstruction denoise
model_id = 'du' if n_args < 3 else sys.argv[2] 

# The loss function, 'L1' or 'SSIML1'
loss_name = 'SSIML1' if n_args < 4 else sys.argv[3] 
# The number of sampling angles
n_smp_angles = 60 if n_args < 5 else int(sys.argv[4])
# SSIML1 alpha 
ssiml1_alpha = 0.6 if n_args < 6 else float(sys.argv[5])
# seed
seed = 8051 if n_args < 7 else int(sys.argv[6])
# main directory
main_dir = 'D:/Data/MIDRC/' if n_args < 8 else sys.argv[7]

print('pre_model_id', pre_model_id)
